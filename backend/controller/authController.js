...
try {
    ...
    if(!user) {
        await userModel.create({name,userName,password: hashedPassword, image});
...
return res.status(200).json({logdIn: "logdIn successfully", token, userid: user._id})
    }
    else {
        ...
        try {
            ...
            if (user) {
                ...
                if(matched) {
                    ...
                    await userModel.updateOne({_id: req.user._id}, {$set: {password: hashedPassword}})
